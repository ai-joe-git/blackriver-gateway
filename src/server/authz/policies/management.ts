...
function requestPeerAddress(ctx: PolicyContext): string | null {
  ...
  const stamped = resolveStampedPeer(
    ...
    process.env.BlackRiver Gateway_PEER_STAMP_TOKEN
  );
...
const WS_BRIDGE_SECRET_HEADER = "x-BlackRiver Gateway-ws-bridge-secret";
...
// server-ws.mjs (BlackRiver Gateway_WS_BRIDGE_SECRET). Without this carve-out the MANAGEMENT
...
function isValidWsBridgeRequest(ctx: PolicyContext): boolean {
  ...
  const expected = process.env.BlackRiver Gateway_WS_BRIDGE_SECRET || "";
