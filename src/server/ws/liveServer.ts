...
function handleInternalEventRequest(req: IncomingMessage, res: ServerResponse): void {
  if (req.method !== "POST" || req.url !== "/__BlackRiver Gateway_event") {
...
//   BlackRiver Gateway_ENABLE_LIVE_WS=0   (or "false")
//
...
export function isLiveWsEnabled(): boolean {
  const v = process.env.BlackRiver Gateway_ENABLE_LIVE_WS;
