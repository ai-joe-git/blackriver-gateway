...
import { getQuotaScopeLabelForProvider } from "@BlackRiver Gateway/open-sse/services/antigravityQuotaFamily.ts";
import {
...
import { isLocalProvider } from "@BlackRiver Gateway/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "@BlackRiver Gateway/open-sse/config/constants.ts";
import {
  preflightQuota,
  isQuotaPreflightEnabled,
} from "@BlackRiver Gateway/open-sse/services/quotaPreflight.ts";
import { resolveResilienceSettings } from "@/lib/resilience/settings";
import { resolveModelLockoutSettings } from "@/lib/resilience/modelLockoutSettings";
import { syncHealthFromDB, type KeyHealth } from "@BlackRiver Gateway/open-sse/services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@BlackRiver Gateway/open-sse/services/errorClassifier.ts";
...
} from "@BlackRiver Gateway/open-sse/config/codexQuotaScopes.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-BlackRiver Gateway-session"),
    "header"
  );
 ...
 * - `Authorization: Bearer <key>` (OpenAI / BlackRiver Gateway / Codex CLI / Bearer clients)
 ...
 * rejected by per-route gates that compare against BlackRiver Gateway keys.
 *
 ...
 * Feature #1350: Supports BlackRiver Gateway_API_KEY / ROUTER_API_KEY env vars as
...
export async function isValidApiKey(apiKey: string) {
  ...
  const envKey = process.env.BlackRiver Gateway_API_KEY || process.env.ROUTER_API_KEY;
