...
export function getRuntimePorts(): RuntimePorts {
  // BlackRiver Gateway_PORT preserves the user's canonical PORT in wrapped runtimes
  // where Next.js requires process.env.PORT to be the dashboard listener port.
  const basePort = parsePort(process.env.BlackRiver Gateway_PORT || process.env.PORT, DEFAULT_PORT);
