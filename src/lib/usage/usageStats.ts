 ...
 * Token Plan), where the upstream exposes no API-key usage endpoint — BlackRiver Gateway
 * counts the tokens it routed and compares against the known monthly limit.
 * Only reflects traffic that went THROUGH BlackRiver Gateway (not the provider's panel).
 */
...
/**
 * Total USD spend BlackRiver Gateway has recorded for a single provider connection, across all time
 ...
 * total. Only reflects traffic that went THROUGH BlackRiver Gateway, not the provider's own dashboard. Used
