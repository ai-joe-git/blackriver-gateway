...
const LOCAL_USAGE_MODEL = "BlackRiver Gateway/local-usage";
  ...
  try {
    ...
    return {
      preferredConnectionId:
        ...
        readHeader(request, "x-BlackRiver Gateway-connection")?.trim() ||
        null,
      preferredProvider: url.searchParams.get("provider")?.trim() || null,
    };
  } catch {
    return {
      preferredConnectionId: readHeader(request, "x-BlackRiver Gateway-connection")?.trim() || null,
