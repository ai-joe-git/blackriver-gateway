 ...
 *      BlackRiver Gateway itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      BlackRiver Gateway's own `/v1` so the vision model can be any provider
 *      registered in BlackRiver Gateway (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. BlackRiver Gateway self-loop (`http://localhost:20128/v1`) — auto-detected when
 *      the model uses a known BlackRiver Gateway-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require BlackRiver Gateway self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through BlackRiver Gateway's own router,
  // not through a direct OpenAI/Anthropic endpoint. Use the operator-configured
  // port via BlackRiver Gateway_PORT / PORT env vars, falling back to the default 20128.
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // BlackRiver Gateway itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the BlackRiver Gateway self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so BlackRiver Gateway can resolve the
      ...
      // When routing through BlackRiver Gateway's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_BlackRiver Gateway as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_BlackRiver Gateway";
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${selfLoopApiKey}`,
      };
      if (useFullModelId) {
        headers["x-BlackRiver Gateway-disabled-guardrails"] = "vision-bridge";
      }
