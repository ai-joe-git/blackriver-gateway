 ...
 * types, the provider-id mapping table, and the raw→BlackRiver Gateway transform
...
// ─── Provider mapping: models.dev provider ID → BlackRiver Gateway provider IDs/aliases ──
//
// models.dev uses canonical provider IDs (e.g. "openai", "anthropic", "google").
// BlackRiver Gateway uses both full IDs and short aliases (e.g. "cc" for claude, "cx" for codex).
// We map each models.dev provider to ALL BlackRiver Gateway identifiers that should receive
...
export const MODELS_DEV_PROVIDER_MAP: Record<string, string[]> = {
  ...
  // that is the historical alias pair. But BlackRiver Gateway's catalog & combo targets
  ...
  // Additional providers that may overlap with BlackRiver Gateway
  alibaba: ["ali", "alibaba"],
...
/**
 * Map a models.dev provider ID to BlackRiver Gateway provider IDs.
...
/**
 * Transform models.dev raw data → BlackRiver Gateway PricingByProvider format.
 *
 * models.dev costs are already in $/1M tokens (same as BlackRiver Gateway format).
...
export function transformModelsDevToPricing(raw: ModelsDevData): PricingByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    const BlackRiver GatewayProviders = mapProviderId(providerId);
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      // Write to ALL mapped BlackRiver Gateway providers
      for (const omniProvider of BlackRiver GatewayProviders) {
