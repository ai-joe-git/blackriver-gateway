...
const HEADER_NAME = "x-BlackRiver Gateway-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local BlackRiver Gateway CLI.
 * Only accepted from loopback IPs. Disabled via BlackRiver Gateway_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.BlackRiver Gateway_DISABLE_CLI_TOKEN === "true") return false;
