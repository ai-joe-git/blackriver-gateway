...
/**
 * Endpoint suportado pelo Playground Studio. Reflete os 13 endpoints da API BlackRiver Gateway
...
export const API_KEY_PLACEHOLDER = "$BlackRiver Gateway_API_KEY";
...
function buildCurlSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `# Set your API key: export BlackRiver Gateway_API_KEY="your-key-here"`,
    `curl -s -X POST \\`,
    `  "${url}" \\`,
...
function buildPythonSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    ...
    `import requests`,
    ``,
    `api_key = os.environ["BlackRiver Gateway_API_KEY"]`,
    ``,
    `url = "${url}"`,
    `headers = {`,
...
function buildTypescriptSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `// Set your API key: export ${API_KEY_PLACEHOLDER}="your-key-here"`,
    `const apiKey = process.env.BlackRiver Gateway_API_KEY ?? "";`,
    ``,
    `const url = "${url}";`,
    `const body = ${bodyJson};`,
    ``,
