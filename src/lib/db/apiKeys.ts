...
import { resolveModelAlias } from "@BlackRiver Gateway/open-sse/services/modelDeprecation.ts";
...
function isRedisAuthCacheEnabled(): boolean {
  return (
    process.env.BlackRiver Gateway_DISABLE_REDIS_AUTH_CACHE !== "1" &&
...
/**
 * Select an API key for internal BlackRiver Gateway operations (combo health checks,
...
export async function getApiKeyMetadata(
  ...
  if (isConfiguredEnvApiKey(key)) {
    ...
    // The deployment-time env key (`BlackRiver Gateway_API_KEY` / `ROUTER_API_KEY`)
