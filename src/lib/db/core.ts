  ...
  try {
    ...
    console.warn(
      `[DB] Cannot create data directory '${DATA_DIR}': ${msg}\n` +
        ...
        `[DB]   DATA_DIR=/path/to/writable/dir BlackRiver Gateway`
    );
  }
}
...
declare global {
  var __BlackRiver GatewayDb: SqliteAdapter | undefined;
}

function getDb(): SqliteDatabase | null {
  return globalThis.__BlackRiver GatewayDb ?? null;
}

function setDb(db: SqliteDatabase | null): void {
  if (db) {
    globalThis.__BlackRiver GatewayDb = db;
  } else {
    delete globalThis.__BlackRiver GatewayDb;
  }
}
...
function shouldRunStartupDbHealthCheck(): boolean {
  if (process.env.BlackRiver Gateway_FORCE_DB_HEALTHCHECK === "1") return true;
...
function getDbHealthCheckIntervalMs(): number {
  const rawValue = process.env.BlackRiver Gateway_DB_HEALTHCHECK_INTERVAL_MS;
...
function startDbHealthCheckScheduler(db: SqliteDatabase) {
  ...
  dbHealthCheckTimer = setInterval(() => {
    try {
      ...
      runDbHealthCheck(db, {
        autoRepair: true,
        skipIntegrityCheck: process.env.BlackRiver Gateway_SKIP_DB_HEALTHCHECK === "1",
...
export function getDbInstance(): SqliteDatabase {
  ...
  db.exec(`
    ...
    INSERT OR IGNORE INTO _BlackRiver Gateway_migrations (version, name)
  ...
  if (shouldRunStartupDbHealthCheck()) {
    const skipIntegrityCheck = process.env.BlackRiver Gateway_SKIP_DB_HEALTHCHECK === "1";
    if (skipIntegrityCheck) {
      console.log("[DB] Health check skipped (BlackRiver Gateway_SKIP_DB_HEALTHCHECK=1)");
    }
    runDbHealthCheck(db, {
      autoRepair: true,
      expectedSchemaVersion: "1",
 ...
 * can place it in ~/.BlackRiver Gateway/runtime/ without blocking a synchronous caller.
 *
