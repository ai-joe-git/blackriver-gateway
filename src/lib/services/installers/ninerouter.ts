...
export async function install(version = "latest"): Promise<InstallResult> {
  ...
  if (!fs.existsSync(hostPkgPath)) {
    fs.writeFileSync(
      hostPkgPath,
      JSON.stringify(
        { name: "BlackRiver Gateway-9router-host", version: "0.0.0", private: true, dependencies: {} },
        null,
        2
      ),
      "utf8"
...
export function resolveSpawnArgs(apiKey: string, port: number): SpawnArgs {
  ...
  return {
    ...
    env: {
      ...
      // BlackRiver Gateway generates this secret and can derive valid keys from it.
