/**
 * BlackRiver Gateway Copilot — Chat Engine
 *
...
function getKnowledgeResponse(query: string): string | null {
  ...
  if (
    ...
    return `## BlackRiver Gateway Architecture
...
Use \`createCombo\` tool or \`runBlackRiver GatewayCli\` to create them.`;
  }
...
BlackRiver Gateway supports 212+ providers across categories:
...
I can use CodeGraph to explore the BlackRiver Gateway codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## BlackRiver Gateway Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`BlackRiver Gateway health\`
- "CLI list-combos" → ejecuta \`BlackRiver Gateway list-combos\`
- "CLI set-budget 10" → ejecuta \`BlackRiver Gateway set-budget 10\`

### Conocimiento
- "Cómo funciona BlackRiver Gateway?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with BlackRiver Gateway.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runBlackRiver GatewayCli") {
    const tool = getCopilotTool("runBlackRiver GatewayCli");
    if (!tool) return { message: "Error: CLI executor not found." };

    const result = await tool.handler(intent.args);
    return {
      message: result,
      toolCalls: [{ name: "runBlackRiver GatewayCli", args: intent.args, result }],
    };
  }
