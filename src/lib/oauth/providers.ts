 ...
 * navigate back to BlackRiver Gateway instead of stalling on localhost.
 */
export function resolveBrowserOAuthRedirectUri(
  ...
  const publicBaseUrl =
    normalizeBaseUrl(env.NEXT_PUBLIC_BASE_URL) || normalizeBaseUrl(env.BlackRiver Gateway_PUBLIC_BASE_URL);
