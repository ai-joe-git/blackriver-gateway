 ...
 * ONLY from `npm info BlackRiver Gateway version --json` (the `npm` CLI binary). When that binary
...
const NPM_REGISTRY_LATEST_URL = "https://registry.npmjs.org/BlackRiver Gateway/latest";
...
const GITHUB_RELEASES_LATEST_URL =
  "https://api.github.com/repos/diegosouzapw/BlackRiver Gateway/releases/latest";
...
export async function getLatestVersionFromNpmCli(): Promise<string | null> {
  try {
    ...
    const { stdout } = await execFileAsync(
      "npm",
      ["info", "BlackRiver Gateway", "version", "--json"],
  ...
  try {
    const res = await fetchImpl(GITHUB_RELEASES_LATEST_URL, {
      ...
      headers: {
        ...
        "User-Agent": "BlackRiver Gateway-version-check",
