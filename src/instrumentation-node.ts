...
/**
 * Rename a Node process title so BlackRiver Gateway is identifiable in `ps`/`htop`
...
export function renameProcessTitle(currentTitle: string): string {
  ...
  return `BlackRiver Gateway${currentTitle.slice("next-server".length)}`;
}

function isBackgroundServicesDisabled(): boolean {
  const raw = process.env.BlackRiver Gateway_DISABLE_BACKGROUND_SERVICES;
  if (!raw) return false;
...
export async function registerNodejs(): Promise<void> {
  // Rename the process title so BlackRiver Gateway is identifiable in ps/htop instead
  ...
  await import("@BlackRiver Gateway/open-sse/index.ts");
  ...
  // See: https://github.com/diegosouzapw/BlackRiver Gateway/issues/3625 (Part A)
  try {
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@BlackRiver Gateway/open-sse/services/batchProcessor");
    initBatchProcessor();
  ...
  try {
    ...
    if (settings.systemPrompt) {
      const { setSystemPromptConfig } =
        await import("@BlackRiver Gateway/open-sse/services/systemPrompt.ts");
    ...
    const { hydrateThinkingBudgetConfig } = await import(
      "@BlackRiver Gateway/open-sse/services/thinkingBudget.ts"
    );
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    try {
      const { autoRefreshDaemon } = await import("@BlackRiver Gateway/open-sse/services/autoRefreshDaemon");
    ...
    // daemon on import (gated by BlackRiver Gateway_ENABLE_LIVE_WS, default ON) — but NOTHING
    ...
    // ever pulled it into a module graph, so in the published `BlackRiver Gateway` bin the
