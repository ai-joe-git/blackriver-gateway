/**
 * Build the BlackRiver Gateway `customModels` entries for Factory Droid's `settings.json`.
 *
 * Ported from upstream PR decolua/9router#618 (author Anurag Saxena) — multi-model
 * support for the Factory Droid CLI tool. Adapted to BlackRiver Gateway branding
 * (`custom:BlackRiver Gateway-<i>`) and extracted as a pure helper so the route-handler
...
export function buildDroidCustomModels(
  ...
  if (defaultIndex > 0 && entries[defaultIndex]) {
    ...
    entries.forEach((e, i) => {
      e.index = i;
      e.id = `custom:BlackRiver Gateway-${i}`;
    });
  }
...
/** True when a `customModels` entry was written by BlackRiver Gateway (any index). */
export function isBlackRiver GatewayCustomModel(entry: { id?: unknown } | null | undefined): boolean {
  return typeof entry?.id === "string" && entry.id.startsWith("custom:BlackRiver Gateway");
}
