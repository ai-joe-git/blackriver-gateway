...
export const PRIVATE_PROVIDER_URLS_ENV = "BlackRiver Gateway_ALLOW_PRIVATE_PROVIDER_URLS";
...
// cloud-metadata endpoints stay blocked. Defaults ON (BlackRiver Gateway is local-first); operators
// who only use public providers can disable it to restore strict SSRF blocking.
export const LOCAL_PROVIDER_URLS_ENV = "BlackRiver Gateway_ALLOW_LOCAL_PROVIDER_URLS";
 ...
 * provider URLs (`BlackRiver Gateway_ALLOW_PRIVATE_PROVIDER_URLS`, default OFF). Protocol and
 ...
 * (BlackRiver Gateway is local-first — local OpenAI-compatible providers should validate out of the
 * box). Disable via the `BlackRiver Gateway_ALLOW_LOCAL_PROVIDER_URLS` flag (DB toggle or env) to
