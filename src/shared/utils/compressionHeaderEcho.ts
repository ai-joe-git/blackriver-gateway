 ...
 *
 * When a request carries `x-BlackRiver Gateway-compression`, docs promise the response echoes
 * `X-BlackRiver Gateway-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-BlackRiver Gateway-compression";
const COMPRESSION_RESPONSE_HEADER = BlackRiver Gateway_RESPONSE_HEADERS.compression;
...
/**
 * Wrap a Response so it carries `X-BlackRiver Gateway-Compression: <mode>; source=request-header`
 * when the request supplied `x-BlackRiver Gateway-compression` and the inner pipeline did not
