...
/**
 * Check if the config file contains BlackRiver Gateway settings.
 */
const hasBlackRiver GatewayConfig = (settings: Record<string, unknown> | null): boolean => {
  ...
  return (
    ...
    settings._managedBy === "BlackRiver Gateway"
  );
};
...
// POST — write BlackRiver Gateway settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge BlackRiver Gateway settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove BlackRiver Gateway settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove BlackRiver Gateway-managed fields
    ...
    return NextResponse.json({ success: true, message: "Smelt BlackRiver Gateway settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
