...
// POST — write BlackRiver Gateway settings to Pi config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge BlackRiver Gateway settings (pi uses OpenAI-compatible config)
...
// DELETE — remove BlackRiver Gateway settings from Pi config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Pi BlackRiver Gateway settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
