...
import {
  ...
  isBlackRiver GatewayCustomModel,
...
// Check if settings has BlackRiver Gateway customModels.
// Multi-model entries are stored as `custom:BlackRiver Gateway-0`, `custom:BlackRiver Gateway-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasBlackRiver GatewayConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isBlackRiver GatewayCustomModel);
};
...
// POST - Update BlackRiver Gateway customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing BlackRiver Gateway config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isBlackRiver GatewayCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend BlackRiver Gateway entries (one per requested model)
...
// DELETE - Remove BlackRiver Gateway customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove BlackRiver Gateway customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isBlackRiver GatewayCustomModel(m));
