...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const config =
      validation.data as import("@BlackRiver Gateway/open-sse/services/taskAwareRouter.ts").TaskRoutingConfig;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (actionRequest.action === "detect") {
      const { detectTaskType } = await import("@BlackRiver Gateway/open-sse/services/taskAwareRouter.ts");
