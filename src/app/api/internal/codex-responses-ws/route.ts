...
import { z } from "zod";
import { CodexExecutor } from "@BlackRiver Gateway/open-sse/executors/codex.ts";
...
import { logger } from "@BlackRiver Gateway/open-sse/utils/logger.ts";
import { resolveProxy } from "@BlackRiver Gateway/open-sse/utils/networkProxy.ts";
import { proxyConfigToUrl } from "@BlackRiver Gateway/open-sse/utils/proxyDispatcher.ts";
...
function getRequestPath(body: JsonRecord): string {
  ...
  try {
    ...
    return new URL(requestUrl, "http://BlackRiver Gateway.local").pathname;
  } catch {
    return "/v1/responses";
  }
}
...
function getBridgeSecret(): string {
  return process.env.BlackRiver Gateway_WS_BRIDGE_SECRET || "";
}
...
function getAuthRequest(body: JsonRecord): Request {
  ...
  const url = new URL(requestUrl, "http://BlackRiver Gateway.local");
...
async function prepare(body: JsonRecord) {
  // Global kill-switch (feature flag BlackRiver Gateway_CODEX_WS_ENABLED, default ON).
  // When disabled, the public Responses-over-WebSocket endpoint is unavailable.
  if (!isFeatureFlagEnabled("BlackRiver Gateway_CODEX_WS_ENABLED")) {
  ...
  // Responses WebSocket too. The downstream client→BlackRiver Gateway hop works, but the
...
export async function POST(request: Request) {
  ...
  const receivedSecret = request.headers.get("x-BlackRiver Gateway-ws-bridge-secret") || "";
