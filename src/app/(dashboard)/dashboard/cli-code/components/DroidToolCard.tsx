...
export default function DroidToolCard({
  ...
  // (#618) Match any custom:BlackRiver Gateway-<i> entry (multi-model).
  const isBlackRiver GatewayEntry = (m) => typeof m?.id === "string" && m.id.startsWith("custom:BlackRiver Gateway");

  const getConfigStatus = () => {
    if (!cliReady) return null;
    const currentConfig = droidStatus.settings?.customModels?.find(isBlackRiver GatewayEntry);
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      // (#618) Pre-fill the multi-model list from every custom:BlackRiver Gateway-<i>
      // entry, preserving the original index order.
      const existing = (droidStatus.settings?.customModels || [])
        .filter(isBlackRiver GatewayEntry)
        .slice()
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      customModels: modelsForPreview.map((m, i) => ({
        model: m,
        id: `custom:BlackRiver Gateway-${i}`,
        index: i,
        baseUrl: getEffectiveBaseUrl(),
        apiKey: keyToDisplay,
        displayName: m,
          ...
          {!checkingDroid && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL — first BlackRiver Gateway entry, any index (#618) */}
                {droidStatus?.settings?.customModels?.find(isBlackRiver GatewayEntry)?.baseUrl && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!droidStatus?.hasBlackRiver Gateway}
                  loading={restoring}
                >
